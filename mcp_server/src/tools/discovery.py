"""Discovery: search, list, and inspect assets."""
...
def _hits(response: dict[str, Any]) -> list[dict[str, Any]]:
    return [h["_source"] for h in response.get("hits", {}).get("hits", [])]


@tool
def search_assets(query: str, entity_type: str = "table", limit: int = 20) -> dict[str, Any]:
    """Full-text search across the catalog. Use "*" for a broad listing."""
    resp = get_client().search(query=query or "*", index=_index_for(entity_type), size=limit)
...
def get_asset(fqn: str, entity_type: str = "tables") -> dict[str, Any]:
    ...
    return get_client().get_entity_by_fqn(
        entity_type, fqn,
        fields="owners,tags,columns,description,domain,dataProducts,tier,usageSummary",
    )


@tool
def get_asset_schema(fqn: str) -> dict[str, Any]:
    """Return columns/schema of a table with descriptions and types."""
...
@tool
def list_assets_by_owner(owner_name: str, entity_type: str = "table", limit: int = 50) -> dict[str, Any]:
    """List assets owned by a given user or team display name."""
    resp = get_client().search(
        query=f'owners.displayName:"{owner_name}"', index=_index_for(entity_type), size=limit,
    )
    return {"owner": owner_name, "count": len(_hits(resp)), "assets": _hits(resp)}


@tool
def list_undocumented_assets(entity_type: str = "table", owner_name: str | None = None, limit: int = 50) -> dict[str, Any]:
    """List assets missing a description (documentation backlog)."""
    parts = ["NOT description:*"]
    if owner_name:
        parts.append(f'owners.displayName:"{owner_name}"')
    resp = get_client().search(query=" AND ".join(parts), index=_index_for(entity_type), size=limit)
    assets = [
...
def list_assets_by_domain(domain_fqn: str, entity_type: str = "table", limit: int = 50) -> dict[str, Any]:
    ...
    resp = get_client().search(
        query=f'domain.fullyQualifiedName:"{domain_fqn}"', index=_index_for(entity_type), size=limit,
    )
...
def list_assets_by_tag(tag_fqn: str, entity_type: str = "table", limit: int = 50) -> dict[str, Any]:
    """List assets carrying a Classification tag (e.g. PII.Sensitive)."""
    resp = get_client().search(
        query=f'tags.tagFQN:"{tag_fqn}"', index=_index_for(entity_type), size=limit,
    )
