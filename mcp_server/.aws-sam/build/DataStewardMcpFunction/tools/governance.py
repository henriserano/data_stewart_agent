"""Governance: ownership, domains, teams."""
...
def list_orphan_assets(entity_type: str = "table", limit: int = 50) -> dict[str, Any]:
    """List assets without any owner."""
    resp = get_client().search(query="NOT owners.name:*", index=f"{entity_type}_search_index", size=limit)
    hits = [h["_source"] for h in resp.get("hits", {}).get("hits", [])]
    return {"count": len(hits), "assets": hits}


@tool
def propose_ownership(fqn: str, owner_name: str, owner_type: str = "team", confirm: bool = False, entity_type: str = "tables") -> dict[str, Any]:
    """Assign a user or team as owner. Requires confirm=True."""
    client = get_client()
    entity = client.get_entity_by_fqn(entity_type, fqn, fields="owners")
    owner = client.get_entity_by_fqn("teams" if owner_type == "team" else "users", owner_name)
...
def get_stewardship_dashboard(owner_name: str) -> dict[str, Any]:
    """One-shot snapshot of an owner's perimeter and its health signals."""
    resp = get_client().search(
        query=f'owners.displayName:"{owner_name}"', index="table_search_index", size=200,
    )
    hits = [h["_source"] for h in resp.get("hits", {}).get("hits", [])]
